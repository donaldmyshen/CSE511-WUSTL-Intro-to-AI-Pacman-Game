...
class QLearningAgent(ReinforcementAgent):
  ...
  def getQValue(self, state, action):
    ...
    if self.QValues[state] == 0: self.QValues[state] = util.Counter()  
  ...
  def getValue(self, state):
    ...
    if qvalue == 0: qvalue = util.Counter()
    return qvalue[qvalue.argMax()]
  ...
  def getPolicy(self, state):
    ...
    bestList = [i for i,j in enumerate(QValues) if j == max(QValues)]
    # random choose
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def getQValue(self, state, action):
    ...
    "*** YOUR CODE HERE ***"
    #util.raiseNotDefined()
    self.feature = SimpleExtractor().getFeatures(state,action)   
    self.featureList = self.feature.keys()
    QValue = 0
    for feature in self.featureList:
        QValue += self.feature[feature] * self.weights[feature]
    return QValue
  ...
  def update(self, state, action, nextState, reward):
    ...
    for feature in self.featureList:
        self.weights[feature] += (self.alpha * update * self.feature[feature])
    return self.weights

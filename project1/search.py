...
def uniformCostSearch(problem):
    ...
    visited = []
    while Fringe.isEmpty() != True: 
        popState, popDirection,popCost = Fringe.pop()
        if popState in visited:
            continue
        if problem.isGoalState(popState):
            return popDirection
        ...
        for state, direction, cost in problem.getSuccessors(popState):
            if state in visited:
                continue
            Fringe.push((state, popDirection+[direction],popCost+cost),popCost+cost)
    return []
    # util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while Fringe.isEmpty() != True: 
	    ...
	    h = heuristic(state,problem)
            Fringe.push((state, popDirection+[direction],popCost+cost),popCost+cost+h)
    return []
